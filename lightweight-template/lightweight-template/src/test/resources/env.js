...
(function(){
	//window._loadDOM = function(file){
	//	window.document = new DOMDocument("<embed>", dom);
	//}
	...
	window.DOMDocument = function(file, dom){
		...
		if (!dom){
			...
			this._dom = new Packages.nu.validator.htmlparser.dom.HtmlDocumentBuilder().parse(file);
		}else{
			this._dom = dom;
		}
